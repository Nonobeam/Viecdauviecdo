...
import { deleteSkill, getSkills, getUserById, uploadAvatar } from "@/utils/userApi"
...
const Profile = () => {
  const { user, setUserData, userData, updated, setUpdated, loading, error, isTokenValid } = useUser()
  ...
  // Subscription state
  const [currentPlan, setCurrentPlan] = useState("FREE") // Initialize with "FREE" or default
  ...
  // Effect to read 'pack' from JWT and set currentPlan
  useEffect(() => {
    const token = localStorage.getItem("token")
    console.log("Profile.jsx - Token from localStorage:", token) // Debug log for token
    if (token) {
      const decodedToken = decodeToken(token)
      console.log("Profile.jsx - Decoded Token:", decodedToken) // Debug log for decoded token
      if (decodedToken && decodedToken.pack) {
        setCurrentPlan(decodedToken.pack.toUpperCase()) // Set currentPlan from decoded token
        console.log("Profile.jsx - Current Plan from Token:", decodedToken.pack.toUpperCase())
      } else {
        // If no pack found in token, explicitly set to FREE
        setCurrentPlan("FREE")
        console.log("Profile.jsx - No pack found in token, setting currentPlan to FREE.")
      }
    } else {
      // If no token found, explicitly set to FREE
      setCurrentPlan("FREE")
      console.log("Profile.jsx - No token found, setting currentPlan to FREE.")
    }
  }, [user]) // Depend on 'user' to re-run if user data changes (e.g., after login)
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-purple-50 via-blue-50 to-indigo-50 flex items-center justify-center">
        <div className="text-center max-w-md">
          ...
          <h2 className="text-xl font-semibold text-gray-800 mb-2">Phiên đăng nhập đã hết hạn</h2>
  ...
  const fetchSkills = async () => {
    if (!user?.user_id) return // Use optional chaining for user
  ...
  const handleDeleteSkill = async (skillToDelete) => {
    ...
    try {
      ...
      Swal.fire({
        ...
        text:
          error.status === 401 ? "Phiên đăng nhập đã hết hạn." : "Đã có lỗi xảy ra khi xóa kỹ năng. Vui lòng thử lại.",
      })
    }
  }
  ...
  const handleAvatarChange = async (file) => {
    ...
    try {
      ...
      Swal.fire({
        ...
        text: err.status === 401 ? "Phiên đăng nhập đã hết hạn." : "Đã có lỗi xảy ra khi cập nhật ảnh đại diện.",
      })
    }
  }

  const handlePlanSelect = (planId) => {
    setCurrentPlan(planId.toUpperCase()) // Ensure it's uppercase to match JWT 'pack'
    ...
    <div className="min-h-screen bg-gradient-to-br from-purple-50 via-blue-50 to-indigo-50">
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        {isExpiringSoon && (
          <div className="mb-4 p-3 bg-orange-50 border border-orange-200 rounded-lg">
            <div className="flex items-center gap-2 text-orange-800">
              ...
              <span className="text-sm">Phiên đăng nhập sẽ hết hạn trong {formattedTimeUntilExpiration}</span>
            </div>
          </div>
        )}
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
          ...
          {/* Main Content */}
          <MainContent userInformation={userInformation} isOwner={isOwner} isTokenValid={isTokenValid} user={user} />
          {/* Right Sidebar - Subscription */}
          <div className="lg:col-span-1">
            <div className="sticky top-8">
              <CompactSubscriptionCard
                currentPlan={currentPlan} // Pass currentPlan state
                isTokenValid={isTokenValid}
